package {
  ...
  public class Share extends Sprite {
    ...
    private static const SHARE_TEXT: TextFormat = new TextFormat("Helvetica, Arial", 42, 0xFFFFFF, true);

    private static const TWITTER_ICON: String = "\ue601";
    private static const TWITTER_COLOR: uint = 0x01ACED;

    private static const LINK_ICON: String = "\ue600";
    ...
    public function Share(url: String, width: uint, height: uint) {
      ...
      var twitter: Sprite = Layout.scale(new ShareButton("Twitter", TWITTER_ICON, url, TWITTER_COLOR), 0.6);
      var href: Sprite = Layout.scale(new ShareButton("Link", LINK_ICON, url, LINK_COLOR), 0.6);

      var background: Sprite = Helpers.fill(width, height, BACKGROUND_COLOR, BACKGROUND_OPACITY);
      var shareText: Sprite = Layout.absolute(10, 10, share);
